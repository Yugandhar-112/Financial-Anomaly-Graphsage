...
def create_mock_transaction_data(num_nodes=1000, num_edges=4000):
    """Generates structured synthetic financial transaction graphs with homophily."""
    np.random.seed(42)
    
    # 10 continuous transaction attributes
    features = np.random.normal(loc=0.0, scale=1.0, size=(num_nodes, 10))
    ...
    features[anomaly_indices] += np.random.normal(loc=1.5, scale=0.5, size=(len(anomaly_indices), 10))
    
    edge_src = []
    edge_dst = []
    
    # Build structural homophily: Anomalies link to other anomalies (fraud rings)
    for i in range(num_edges):
        if np.random.rand() < 0.4 and len(anomaly_indices) > 1:
            # 40% chance to force an anomaly-to-anomaly coordination edge
            src = np.random.choice(anomaly_indices)
            dst = np.random.choice(anomaly_indices)
            while src == dst:
                dst = np.random.choice(anomaly_indices)
        else:
